...
template <typename T>
static inline T GetFieldValue(const uint8_t *buf, const ClassLayout &layout, uint16_t fieldId) {
...
static void PopulateEntityStateProto(
	portal2_harness::EntityState *protoState,
	int entityIndex,
	...
	Vector position = GetFieldValue<Vector>(currentBuf, layout, HDEM_FIELD_ORIGIN);
	...
	Vector velocity = GetFieldValue<Vector>(currentBuf, layout, HDEM_FIELD_VELOCITY);
	...
	for (const auto &fs : layout.fields) {
		if (fs.fieldId == HDEM_FIELD_ORIGIN || fs.fieldId == HDEM_FIELD_ANGLES || fs.fieldId == HDEM_FIELD_VELOCITY) {
			continue;
		}
		...
		if (lastFieldValues && lastFieldValues->size() == slot.fieldBufSize) {
			if (std::memcmp(bytes, lastFieldValues->data() + fs.dstOffset, fs.size) == 0) {
				continue;
			}
		}
...
bool Portal2HarnessImpl::InternalObserve(portal2_harness::GameState *response) {
	...
	if (engine->demoplayer->IsPlaying()) {
		...
		if (harness && harness->hdemReader && harness->hdemReader->IsOpen()) {
			...
			harness->hdemReader->GetSnapshot(response->mutable_entity_snapshot(), serverTick);

			// Find the player entity (index 1) in the HDEM snapshot to extract exact server-side state
			...
			if (isAlive) {
				...
				if (!wasSeen || currentVersion != observeLastVersion[i]) {
					...
					PopulateEntityStateProto(
						protoState, i, slot, layout, allFields, isNew ? nullptr : &observeLastState[i]);

					observeLastState[i].assign(slot.fieldBuf.get(), slot.fieldBuf.get() + slot.fieldBufSize);
...
grpc::Status Portal2HarnessImpl::Act(
	grpc::ServerContext *context, const portal2_harness::ActionRequest *request, portal2_harness::ActionResponse *response) {
...
grpc::Status Portal2HarnessImpl::Reset(
	grpc::ServerContext *context, const portal2_harness::ResetRequest *request, portal2_harness::ResetResponse *response) {
...
grpc::Status Portal2HarnessImpl::AgentLoop(
	grpc::ServerContext *context,
	grpc::ServerReaderWriter<portal2_harness::EnvironmentMessage, portal2_harness::AgentMessage> *stream) {
	if (!session->isRunning) {
			...
			if (shm.GetBuffer() != MAP_FAILED && shm.GetSize() > 0 && g_harness_videomode && *g_harness_videomode) {
				...
				Scheduler::OnMainThread([&]() {
					...
					Memory::VMT<void(__rescall *)(void *, int, int, int, int, void *, int)>(
						...
						*g_harness_videomode, 0, 0, sw, sh, shm.GetBuffer(), 2 /* IMAGE_FORMAT_RGB888 */);
...
grpc::Status Portal2HarnessImpl::RenderDemo(
	...
	Scheduler::OnMainThread([&]() {
		...
		if (!harness->rolloutRecorder->Start(outputPath, targetMapName, shmName, sw, sh, 1.0f / engine->GetIPT(), capturePixels)) {
			if (harness->hdemReader) {
				delete harness->hdemReader;
				harness->hdemReader = nullptr;
			}
