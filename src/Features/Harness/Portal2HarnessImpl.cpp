...
grpc::Status Portal2HarnessImpl::Reset(grpc::ServerContext *context, const portal2_harness::ResetRequest *request, portal2_harness::ResetResponse *response) {
	...
	// Wait for the warmup to complete (signaled from PRE_TICK when warmup hits 0)
	{
		...
		harness->resetCV.wait(lock, []() {
			return harness->harnessControlActive.load();
		});
	}
