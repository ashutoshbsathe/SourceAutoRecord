...
uint16_t EntitySnapshotter::GetOrAddField(const std::string &fieldName, HdemFieldType type) {
...
void EntitySnapshotter::DiscoverSendTableFields(const std::string &className, SendTable *table) {
	uint16_t cid = GetOrAddClass(className);
	auto &cls = classes[cid];
	...
	if (added > 0) {
		console->Print("HDEM: %s (%s) -> %d fields (%d from SendTable)\n", className.c_str(), table->m_pNetTableName, (int)cls.fields.size(), added);
	}
}
...
static void ReadFields(EntitySlot &slot, ServerEnt *se, const ClassLayout &layout, uint8_t *dstBuf) {
...
void EntitySnapshotter::Update() {
	...
	for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
		...
		if (std::memcmp(scratchBuf.data(), slot.fieldBuf.get(), slot.fieldBufSize) != 0) {
			slot.changeVersion++;
		}
	}
}
