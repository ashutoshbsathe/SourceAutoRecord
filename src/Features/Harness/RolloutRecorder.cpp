...
bool RolloutRecorder::Start(const std::string &path, const std::string &mapName, const std::string &shmName, int width, int height, float tickrate, bool capturePixels) {
...
void RolloutRecorder::RecordTick(const portal2_harness::GameState &state, const portal2_harness::ActionRequest &action, void *pixels, size_t pixelSize) {
	if (!this->isActive) return;
...
void RolloutRecorder::MapUserCmdToAction(const CUserCmd &cmd, portal2_harness::ActionRequest *req) {
	req->set_num_ticks(1);
	...
	// For ActionRequest, we usually expect a [-1, 1] range representing a "normalized" move,
	// but many agents use raw degrees. We'll pass them as is for now.
