...
- **⭐ `go_to` through portals** — the parked **R5 / fork-B "GoToPlanner portal-edge"** workstream. The A\* planner
  ...
  **R5 recon RAN 2026-07-01** (`sar_harness_portal_reachability_test` on `sp_a2_laser_over_goo`) — **both halves
  confirmed:** the flat planner reads the far side of an open pair as a **disconnected component** (SEVERED,
  symmetric, pristine WALKABLE goal, dz=0, under the cap), and the **engine teleports the player through on
  mouth-contact** (getpos before/after = a clean portal transit — same z/pitch/roll, yaw flipped ~180°). Upshot:
  `go_to` can't smuggle a solve (won't route across a severance) but *can* be derailed if its local VFH path
  grazes an open mouth. **Decision now teed, not recon-blocked:** build the `GoToPlanner` portal edge (fork-B —
  the model places portals, the engine routes through them; a defensive "avoid open mouths" guard rides along) vs
  keep portals `launch`-only. **fork-B (A\*-portal-edge) DROPPED 2026-07-01** — user: routing a path *through*
  the portal is pointless; portals are a first-class Portal 2 mechanic that must interlock with **flings +
  lasers**, not a nav edge. `portal_traversal_edge_design.md` is superseded (kept for its R5-derived substrate
  notes). R5 recon detail: `portal_verb_recon_design.md` §5.3.
- **⭐ UNIFIED PORTAL GRAMMAR RETHINK (2026-07-01) — the live workstream.** 28-agent brainstorm →
  **[portal_unified_grammar_options_dossier.md](portal_unified_grammar_options_dossier.md)** (6 interlocking axes:
  pass_through · flings · fractional-`(u,v)` · laser×portal · portal-naming · percept — all one transform
  `M`=`m_matrixThisToLinked`). **User rulings:** laser verb stays **DUMB** (aim at the mouth, model reasons the
  exit — no pull-back solver) · fractional `(u,v)` on **ALL panels** (gravity-anchored frame) · **SPINE-ONLY**
  this session (defer momentum behind the DIED-terminal) · **fling family reopened** (INTO-portal vs FROM-mark;
  one `launch(via)` rejected). **Traversal/drop family FINAL:** `pass_through(portal)` (walk-self into a
  foot-reachable mouth) · `drop_into(portal, object=None)` (drop self OR a held object into a GROUND portal; no
  `fall_into` verb; throw-into-wall out of scope) · `launch(via=@plate|@gel)` (FROM, deferred). **Laser = FREE**
  once portals are named marks (`redirect_to`/`interpose`/`aim_at` accept `@portal`, no code); only the
  through-portal *seat* (poly-line `interpose`) is gated on the P0 recon. **NEXT = spine build:** (B) portal
...
| `verb_grammar_genie_seed.md` | **⭐ NEXT-SESSION SEED (2026-07-21)** — the "literal genie" harness philosophy + relational point-algebra refactor. Resolves the §6 unified-point-algebra altitude dilemma: let the algebra name anything, the harness grants the literal *spatial* wish and never closes the wish→intent loop (expressive about execution, silent about diagnosis). Locked decisions + open questions for the wrap-up/cleanup brainstorm; paper delayed until it lands. Read before any verb-grammar work. |
