...
class TransformerBlock(nn.Module):
    ...
    def __call__(self, x):
        ...
        y = nn.MultiHeadDotProductAttention(
            num_heads=self.num_heads, name="attn"
        )(y, y)
        x = x + y
...
class IndependentActionHead(nn.Module):
    ...
    def __call__(self, features: jnp.ndarray) -> ActionDistParams:
        ...
        mouse_log_std = self.param(
            "mouse_log_std", nn.initializers.zeros, (2,)
        )
        return ActionDistParams(
            move_fb_logits=move_fb, move_lr_logits=move_lr,
            zoom_logits=zoom, portal_logits=portal,
            buttons_logits=buttons, mouse_mean=mouse_mean,
    ...
    def sample(dist: ActionDistParams, rng: jnp.ndarray):
        ...
        lp = jnp.zeros(dist.move_fb_logits.shape[0])
        ...
        lp_btn = (buttons * jax.nn.log_sigmoid(dist.buttons_logits)
                  + (1 - buttons) * jax.nn.log_sigmoid(-dist.buttons_logits))
        ...
        lp_m = (-0.5 * ((mouse_raw - dist.mouse_mean) / std) ** 2
                - jnp.log(std) - 0.5 * jnp.log(2 * jnp.pi))
        lp = lp + lp_m.sum(axis=-1)

        actions = {"move_fb": move_fb, "move_lr": move_lr, "zoom": zoom,
                   "portal": portal, "buttons": buttons, "mouse": mouse}
        return actions, lp
    ...
    def log_prob(dist: ActionDistParams, actions: Dict[str, jnp.ndarray]):
        ...
        lp = (lp + _clp(dist.move_fb_logits, actions["move_fb"])
        ...
        lp_btn = (b * jax.nn.log_sigmoid(dist.buttons_logits)
                  + (1 - b) * jax.nn.log_sigmoid(-dist.buttons_logits))
        lp = lp + lp_btn.sum(axis=-1)

        std = jnp.exp(dist.mouse_log_std)
        m = actions["mouse"]
        lp_m = (-0.5 * ((m - dist.mouse_mean) / std) ** 2
                - jnp.log(std) - 0.5 * jnp.log(2 * jnp.pi))
        lp = lp + lp_m.sum(axis=-1)
        return lp
    ...
    def entropy(dist: ActionDistParams):
        ent = jnp.zeros(dist.move_fb_logits.shape[0])
        ...
        be = -(p * jax.nn.log_sigmoid(dist.buttons_logits)
...
class ActorCritic(nn.Module):
    ...
    def __call__(self, image_embed, position):
        ...
        x = nn.Dense(self.trunk_hidden, name="trunk_fc1")(fused)
        x = nn.relu(x)
