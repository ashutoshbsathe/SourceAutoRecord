...
uint16_t HdemRecorder::GetOrAddField(const std::string& fieldName,
                                     HdemFieldType type) {
...
void HdemRecorder::DiscoverEntities() {
  ...
  // Universal fields definition (deterministic IDs matching HdemWellKnownField
  // enum)
  ...
  // New universal/specialized properties for cubes / doors / buttons / turrets
  // etc.
  ...
  // Pre-register critical classes guaranteed to be needed during gameplay
  // rollouts, ensuring their metadata schemas are embedded in the header
  // regardless of spawn timing.
...
void HdemRecorder::RecordTick(int tickNumber) {
  ...
  for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
    ...
    if (isFullSnapshot) {
      ...
      if (!cname) cname = "";
      tickBuffer.insert(tickBuffer.end(), cname,
                        cname + std::strlen(cname) + 1);
      fieldsWritten++;
      ...
      if (!tname) tname = "";
      tickBuffer.insert(tickBuffer.end(), tname,
                        tname + std::strlen(tname) + 1);
      fieldsWritten++;
    }
