...
def convert_hdem_to_rollout(hdem_path, rollout_path):
    ...
    try:
        ...
        print(
            f'Conversion complete. Wrote {total_steps} steps, total size: {total_bytes} bytes.'
        )
        return True
    finally:
        reader.close()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Convert .hdem to .rollout (entity-only, offline mode).'
    )
    parser.add_argument('hdem_path', help='Path to input .hdem file')
    parser.add_argument(
        'rollout_path',
        nargs='?',
        help='Path to output .rollout file (default: input_base.rollout)',
    )
