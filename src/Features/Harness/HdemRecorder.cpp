...
void HdemRecorder::DiscoverEntities() {
    ...
    auto addClassSchema = [&](const std::string& cname, bool isPortal) {
    ...
    addClassSchema("prop_portal", true);
    addClassSchema("player", false);
    addClassSchema("portal_player", false);
    addClassSchema("prop_physics", false);
    addClassSchema("prop_dynamic", false);
    addClassSchema("trigger_portal_cleanser", false);
    ...
    for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
        ...
        addClassSchema(className, std::strcmp(className, "prop_portal") == 0);
    }
    ...
    for (int i = 0; i < Offsets::NUM_ENT_ENTRIES; ++i) {
        ...
        for (const auto& field : cls.fields) {
            ...
            case HDEM_FIELD_ACTIVATED:
                ...
                } catch (...) { AppendToBuffer(currentRawState, bool(false)); }
                break;
            ...
            case HDEM_FIELD_LINKEDPORTAL:
                ...
                } catch (...) { AppendToBuffer(currentRawState, uint32_t(0xFFFFFFFF)); }
                break;
            default:
                break;
            }
