...
class HdemReader:
    ...
    def _open(self):
        ...
        try:
            ...
            if magic != HDEM_MAGIC:
                raise ValueError(
                    f'Invalid magic 0x{magic:08X} (expected 0x{HDEM_MAGIC:08X})'
                )
    ...
    def read_next_tick(self):
        ...
        for _ in range(num_ents):
            ...
            if ent_idx not in self.current_entities or (
                eflags & 8
            ):  # HDEM_ENT_FULL_SNAPSHOT
            ...
            for _ in range(num_f):
                ...
                if fid == 11:  # HDEM_FIELD_CLASSNAME
                    ...
                    while ppos < len(payload) and payload[ppos] != 0:
                        ppos += 1
                    ent['class_name'] = payload[start:ppos].decode(
                    ...
                    while ppos < len(payload) and payload[ppos] != 0:
                        ppos += 1
                    ent['target_name'] = payload[start:ppos].decode(
                        'utf-8', errors='replace'
                    )
                    ppos += 1
