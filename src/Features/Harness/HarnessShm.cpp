...
HarnessShm::HarnessShm() : size_(0), fd_(-1), mapped_ptr_(MAP_FAILED), initialized_(false) {}
...
void HarnessShm::Cleanup() {
    ...
    // C++ will write to SHM -> Send gRPC message. Python receives gRPC message -> Reads SHM. 
    // This ensures no race conditions on the memory block without needing complex SHM mutexes.
