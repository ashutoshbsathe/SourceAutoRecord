...
- **Steps:** **reuse Source's `save`/`load`; do NOT build a custom state-restore** (capture is easy, restore — velocities, grab constraints, portal links — is the tar pit). Model-chosen named slots (grammar shape locked at the checkpoint). ~~Cost ~100s of ms~~ — **cost correction (2026-06-20, [astar_routing_design.md](astar_routing_design.md) §5):** every `load` fires `SESSION_START` → a mandatory **256-tick (~4.3 s) harness re-warmup**, so a restore is *seconds*, not ~100s of ms. Fine at turn/macro granularity (a handful per puzzle); fatal as a tight inner search loop. Marks stay stable across restore by construction (A3 canonical-numbering revision). C9 searches *realized* config-space (the engine resolves all dynamics); its static complement is the **offline I/O causal graph** ([offline_map_preprocessing.md](offline_map_preprocessing.md) §2b) — "button X *could* open door Y" parsed once from the BSP, vs C9's "did this *actually* open it" via save/load. The two combined (offline prior prunes + A\* verb primitives + C9 backtrack) = the forward-looking **brute-force solver / experimenter-side solvability+difficulty oracle**, recorded at [offline_map_preprocessing.md](offline_map_preprocessing.md) §7 (PSPACE-complete + ~4.3 s/node → shallow chambers only; record-only).
