...
def read_delimited_proto(file_path):
    ...
    with open(file_path, 'rb') as f:
        ...
        while True:
            ...
            if (act.key_forward or act.key_backward or act.key_left or act.key_right or
                act.key_use or act.key_zoomin or act.key_zoomout or act.key_crouch or
                act.portal_primary or act.portal_secondary or act.key_jump):
            ...
            if count < 10:
                ...
                num_ents = len(step.state.entity_snapshot.entities) if step.state.HasField('entity_snapshot') else 0
        ...
        if entity_snapshot_count > 0:
            print(f'Average entities per snapshot: {total_entities_seen / entity_snapshot_count:.1f}')
            print(f'Unique entity classes tracked: {sorted(list(unique_entity_classes))}')
        print(f'Steps with active action/movement keys: {any_button_pressed_count} / {count}')
